...
The implementation of back-end services are done using a combination of two languages: CDL (CDS' definition language) \cite{cap-cds-cdl} and Java \cite{java}. The concrete implementations are located under the \textit{/srv} folder as illustrated in \autoref{src:15}, in which the underneath components are further explained in \autoref{subsubsec:D-srv-design}. Before that, the general steps/requirements of implementing a service can be checked in \autoref{subsubsec:D-srv-steps}. 
