...
This chapter is dedicated for the potential users. Firstly, a brief introduction of the intention of the solution is explained in \autoref{sec:BusinessContext}. Then in \autoref{sec:GeneralRequisite}, the requisite of opening and using the application is listed. It also gives an introduction to the types of user roles within the solution, to enable the users to navigating through the thesis and find their related information quickly.
Finally in the following sections (\autoref{sec:UdocEndUser}, \autoref{sec:UdocReceptionist}, \autoref{sec:UdocFacilityManager}, \autoref{sec:UdocAdministrator}), which are titled by the roles, collects the use guide of each accessible applications.

\section{Business Context}
\label{sec:BusinessContext}

This solution is designed for the internal usage within SAP supporting the parcel collection service providing to the employees, with the help of which employees can deliver their personal parcels first to the reception, then pick them up at any desired time from the reception. The solution enables the complete digitalization and standardization of the parcel signing and management processes. The solution is realized through six applications as tiles on the Fiori launchpad \cite{flp}: \textbf{My packages}, \textbf{Pickup packages}, \textbf{Register Packages}, \textbf{Manage packages}, \textbf{Manage Companies} and \textbf{Manage Storage} partitioned into 3 sections: \textbf{My Home}, \textbf{Parcel Handling}, \textbf{Administration} (\autoref{fig:ApplicationLaunchScreen}).
...
\begin{compactenum}
    ...
    \item \textit{Register}: Receptionists log into \textbf{Register Packages} and register the newly arrived parcels into system.
    \item \textit{Confirm}: Receptionists confirm the parcel's availability to be picked up in \textbf{Manage Packages} by choosing the storage slots for the arrived parcels. Receptionists place the parcel into selected physical storage slot. (From this point, employees may pickup the parcel.)
...
In order to use any of the deployed solution (the deployment is out of scope of this thesis and should be carry on only after), one has to hold an SAP registered device (computer/tablet/phone) and is able to use a supported browser with stable internet connection. In order to use \textbf{Pickup packages} application, one has to hold an SAP registered smart phone. 

In order to mock use the application in local environment, please first move the the developer documentation (\autoref{ch:impl}) and checkout the tools and mininumn software version. After the proper installations, clone the repository and run the following commands (\autoref{src:5020}).
...
The list items lists \textbf{the type with icon}, \textbf{the delivery company}, \textbf{the registration time} and \textbf{the location} of the packages. 
...
At the end of each line items two (or one depends on the status) actions buttons are displayed for the edition (\autoref{subsubsec:editSlot}) and deletion (\autoref{subsubsec:deleteSlot}) of the slot.
...
\textbf{FM} can edit a slot at "Storage Detail Screen".
In case the \textbf{FM} is at "Storage Detail Screen" and want to edit a slot in the slot list, the \textbf{FM} can use the "Edit" (icon with pen) at the end of the slot row. 
...
\textbf{FM} can delete a storage slot at "Storage Detail Screen". 
...
In case the \textbf{FM} is at "Storage Detail Screen" and want to delete the displayed storage, the "Delete" button (icon displaying a rubbish bin) at the end of the slot list row shall be used. The "Delete" icon is displayed only if the slot is not of status \textit{inuse} (i.e. it is not currently occupied by parcel(s))

Once clicked, a warning message box pops up verifying the \textbf{FM}'s intention to delete the displayed slot. If "Delete" is clicked, the storage will be deleted. The message box will be closed, the slot is removed from the list (and system) and a message toast indicates the success deletion. In case the deletion is not reflected immediately, \textbf{FM} should refresh the page to see the slot disappears from the slot list. If "Cancel" is clicked, the message box is closed and nothing happens.
(\autoref{fig:MSslotObjDeleteGuide})

\begin{figure}[H]
	\centering
	\subcaptionbox{On Click "Delete" - Deletion Verification}{
		\includegraphics[width=0.50\linewidth]{images/user_doc/storage/updatedSlotEditDelete/DeleteSlotMsgBox.png}}
  
    \vspace{10pt}
    
    \subcaptionbox{Post Deletion - Navigated Back}{
		\includegraphics[width=0.95\linewidth]{images/user_doc/storage/updatedSlotEditDelete/DeletedSlotToast.png}}
